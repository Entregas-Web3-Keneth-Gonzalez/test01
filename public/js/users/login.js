...
btnLogin.addEventListener('click', function () {
    auth.signInWithEmailAndPassword(txtEmail.value, txtContra.value)
        .then((userCredential) => {
            ...
            const dt = new Date(); // Obtener la fecha actual para último acceso
            db.collection("datosUsuarios").where('idemp', '==', user.uid).get()
                ...
                .catch(function (FirebaseError) {
                    var mensaje = "Error updating access time: " + FirebaseError;
                    alert(mensaje);
                });
        })
        .catch((error) => {
            var mensaje = "Error al acceder: " + error.message;
            alert(mensaje);
        });
});
